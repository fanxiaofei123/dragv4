...
function getModelAudit(page){ 
	...
	tmp.page = page;
    $.ajax({ 
        url:basePath+"/drag/flow/modelRelease.do",
        type: 'POST',
        dataType: "json",
        ...
        success:function(json){ 
            ...
            $.each(list,function(index,array){ //遍历json数据列 
            	...
            	li +=" <tr class='odd gradeX'>"+
                                            ...
                                            "<td>"+
                                            "<div class='actions'>" ;
                    // "<a class='' href='javascript:;' onclick='modelReleases("+array['id']+")' >发布 </a>"
                                            if(array['type'] == 3){
                                        		li += "<a class='' href='javascript:;' onclick='treebox("+array['id']+")' >发布 </a>";
                                            }
                                        		if(array['type'] == 4){
                                                      li += "<a class='' href='javascript:;' onclick='modelReject("+array['id']+")' >取消发布 </a>" ;
                                              	}
                                              	li += " </div>"+
                                          " </td>"+
                                               "</tr>";
...
function modelReleases(tid,wfid){
...
var setting = {
 ...
 //弹框让用户选择目录
var flowid;
function treebox(wfid){
     flowid=wfid;//工作流id
    $.post(basePath+"/drag/treemode/selectAll.do",function (nodes) {
...
function zTreeOnCheck(event, treeId, treeNode) {
    tid=treeNode.id;
    checketype=treeNode.checked;
    // alert(tid + ", " + treeNode.name + "," + treeNode.checked);
};

$(".yes").on("click",function(){
    if(checketype==true){
        modelReleases(tid,flowid);
        $(".bodyBox").fadeOut(100)
