...
do
  ...
  then
    labelColname=$value
  elif [ $key = "selectFeatures" ]
  then
    featureSelectedNum=$value
...
localInputData=/tmp/workflow-cache/data/`date +%N%S%N`featrueSel_classity_in.csv
localoutputData=/tmp/workflow-cache/data/`date +%N%S%N`featrueSel_classity_out.csv
