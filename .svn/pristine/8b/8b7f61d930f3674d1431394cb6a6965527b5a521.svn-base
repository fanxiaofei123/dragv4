...
public class UploadDataControllerApi {
	// 日志对象
	private static Log logger = LogFactory.getLog(UploadDataControllerApi.class);
	@Autowired
	 ...
	 * 数据长传
	 * @param request
	 * @param currentDir datas/前台用户名/input
	 * @param response
	 * @param userName 后台用户登录名称
	 * @return
	...
	@ResponseBody
	public ResponseResult upload2(HttpServletRequest request, String currentDir, HttpServletResponse response,
		...
		String path = null;
		//String filename = currentDir+'/'+System.currentTimeMillis()+"_"+userName+".csv";
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					...
					if (myFileName.trim() != "") {
						...
						User user = userService.selectByUser(us);
						...
						taskExecutor.execute(new Runnable() {
							...
							public void run() {
								try {
									//判断文件或者目录是否存在
									Boolean flies = iHdfsApi.judgmentFile(currentDir, userName);
									if(flies){
										//创建上传数据的目录
										Boolean create = iHdfsApi.createUserWorkspace(currentDir, userName, true);
										if(create){
											//长传文件到hdfs上
											...
											if (uploadResult) {
												...
												UploadHistory uploadHistory = new UploadHistory(myFileName, new Date(),
														user.getId(), "/"+hdfsUploadDto.getUploadPath().substring(hdfsUploadDto.getUploadPath().indexOf(hdfsUploadDto.getUsr()),hdfsUploadDto.getUploadPath().length()));
		...
		return new ResponseResult(HttpStatus.OK, fileNameList.toString());
	}
	...
	public ResponseResult creDir(String currentDir, String name, HttpSession session) {
		...
		return result ? new ResponseResult(HttpStatus.OK, "创建目录成功")
				: new ResponseResult(HttpStatus.EXPECTATION_FAILED, "创建目录失败");
	}
	...
	public Map<String, Object> shateDatas(HttpSession session, String currentDir) {
		...
		for (HdfsUrlModel receiptList : list) {
			...
			/*
			 * System.out.println(receiptList.getHdfsPath()); List<FileInfo>
			 * fileHdfsList = hdfsFileUtil.list(receiptList.getHdfsPath(),
			 * MailAuthorSetting.HDFS_USER);
			 * hdfs://172.16.0.23:8020/user/govdata/贵阳执行类裁判文书
			 */
		}

		/*
		 * List<String> list = new ArrayList<String>(); list.add("地级行政界线");
		 */
		List<DatasResult> datasResults = new ArrayList<>();
		if (currentDir.equals(MailAuthorSetting.HDFS_USER_DATA)) {
				...
				if (listHdfs.contains(fileName)) {
					...
					if (isdir) {
						Long size = Long.valueOf(
								iHdfsApi.list(currentDir + "/" + fileName, MailAuthorSetting.HDFS_USER).size());
						datasResult.setSize(size + "个文件");
					} else {
						datasResult.setSize(convertFileSize(fileInfo.getLength()));
			...
			for (FileInfo fileInfo : fileStatusList) {
				...
				if (isdir) {
					Long size = Long
							.valueOf(iHdfsApi.list(currentDir + "/" + fileName, MailAuthorSetting.HDFS_USER).size());
					datasResult.setSize(size + "个文件");
				} else {
					datasResult.setSize(convertFileSize(fileInfo.getLength()));
