...
function getModelAudit(page){ 
	...
	tmp.page = page;
    $.ajax({ 
        url:basePath+"/drag/modelRecord/select.do",
        type: 'POST',
        dataType: "json",
        ...
        success:function(json){ 
            ...
            $.each(list,function(index,array){ //遍历json数据列 
            	...
            	li +=" <tr class='odd gradeX'>"+
                                               " <td>"+
                                               "<div class='actions'>"+ array['fName']+ " </div>"+
                                                "</td>"+
                                                "<td>"+
                                                "<div class='actions'>"+ array['tel']+ " </div>"+
                                                "</td>"+
                                                "<td>"+
                                                "<div class='actions'>"+ array['mail']+ " </div>"+
                                                " </td>"+
                                                 "<td>"+
                                                 "<div class='actions'>"+array['typeName']+" </div>"+
                                                  "</td>"+
                                                "<td>"+
                                                "<div class='actions'>"+ array['reason']+ " </div>"+
                                              " </td>"+
                                              "<td>"+
                                        	...
                                        	if(array['type'] == 1){
                                                ...
                                                li += "<a class='' href='javascript:;' onclick='updateRecord("+array['id']+")' >通过 </a>" +
...
function updateRecord(id){
    ...
    data.type = 2;
	 $.ajax({
         url:basePath+"/drag/modelRecord/upType.do",
         type: 'POST',
         data: JSON.stringify(data),
         dataType: "json",
         contentType: "application/json;charset=UTF-8",
