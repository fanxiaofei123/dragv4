        ...
        if(treeNode.name==newName){
            ...
            if (!treeNode.isParent){
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data:{"templateId":id,"templateName":newName,"Tcid":treeNode.pId},
                    success: function (data) {
                        var returndata = JSON.parse(data);
                        if(returndata.code == '417'){
                            toastr.error(returndata.msg);
...
function lookTemplate(templateId) {
    $.ajax({
        ...
        success:function(json){
            ...
            var workFlowName = json.rows[0].workFlowName;
        ...
        // complete:function(){ //查询工作流名
        //     $.ajax({
        //         url:basePath+"/template/selectFlowNameByFlowId.do",
        //         type: 'POST',
        //         // dataType: "json",
        //         data:{"flowId":flowId},
        //         success:function(json){
...
function addTemplateExplainEdit() {
    ...
    if($("#templateName1").val().trim().length>20){
        $("#templateName1 + .title_text").remove();
        $("#templateName1 ").after(" <p class='title_text'>* 请填写20字符串以内。</p>");
        return ;
    }else if($("#templateName1").val().trim().length == 0){
        $("#templateName1 + .title_text").remove();
    ...
    if($("#NewtemplateKeyword").val().trim().length==0 || $("#NewtemplateKeyword").val().trim().length>20){
        $(".selectMagNewtemplateKeyword").html("* 请填写20字符串以内。");
        return;
    }else{
        $(".selectMagNewtemplateKeyword").html("");
    }
    if (nodes.length == 0) {
        // alert("请先选择一个节点");
...
function templateExplainEdit() {
    ...
    if($("#templateName").val().trim().length > 20 || $("#templateName").val().trim().length == 0){
        $(".templateName").html("名称长度最多20个字符");
        return;
    }else{
        $(".templateName").html("");
    }
    if($("#templateKeyword").val().trim().length > 20 || $("#templateKeyword").val().trim().length == 0){
        $(".templateKeyword").html("请填写20个以内字符");
        return;
    }else{
        $(".templateKeyword").html("");
    }
    if (nodes.length == 0) {

    }else if(nodes.length>0){
        treeNode = nodes[0];
        if(treeNode.id && treeNode.isParent == false){
            form.append('flowId',treeNode.id);
        }else{
            $(".selectMag").html("请先选择一条工作流");
            return;
...
function selectTempalateByNameMethod(page) {
    ...
    if(TempalateName==""||TempalateName==null){
        ...
        var zTreeMB = $.fn.zTree.getZTreeObj("treeDemo"),
            nodesMB = zTreeMB.getSelectedNodes(),
            treeNodeMB = nodesMB[0];
        if(treeNodeMB == undefined){
            name = "模板分类";
        }else{
            name = treeNodeMB.name;
        }
        $.ajax({
            url: basePath + "/template/selectAllByAnyThing.do",
            type: 'POST',
            ...
            data: {"page": page,"templateName": TempalateName,"templateCategoryName":name},
            success: function (json) {
                $("#templateTbody").empty();//清空数据区
                curPage = json.curPage; //当前页
                totalPage = json.totalPage; //总页数
